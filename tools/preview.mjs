...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'GET' && (req.url === '/' || req.url === '/index.html')) {
    ...
    catch { res.end('Preview unavailable'); } return;
  }
  if (req.method === 'GET' && req.url === '/api/capabilities') { json(res, 200, { extraction: true }); return; }
  if (req.method !== 'POST' || req.url !== '/api/extract') { json(res, 404, { error: 'Не найдено.' }); return; }
  if (req.headers.origin !== origin || req.headers['x-gorizont-client'] !== 'local-preview') { json(res, 403, { error: 'Разрешены запросы только из локального интерфейса.' }); return; }
  ...
  try {
    ...
    let size = 0;
    const limit = new Transform({ transform(chunk, enc, cb) { size += chunk.length; cb(size > 1024 ** 3 ? new MediaError('Максимальный размер — 1 ГБ.') : null, chunk); } });
    ...
    res.writeHead(200, { 'Content-Type': 'audio/wav', 'Content-Length': result.bytes, 'Content-Disposition': 'attachment; filename="gorizont-audio.wav"', 'X-Audio-Duration': String(result.seconds), 'Cache-Control': 'no-store' });
    await pipeline(createReadStream(output), res);
  } catch (error) {
    if (!res.headersSent && !res.destroyed) json(res, error instanceof MediaError ? 422 : 500, { error: error instanceof MediaError ? error.message : 'Не удалось обработать файл. Проверьте FFmpeg и свободное место.' });
  } finally {
