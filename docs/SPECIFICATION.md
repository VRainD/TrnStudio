...
16 ГБ — проектный бюджет, не подтверждённый расход. На целевой карте замерить VRAM, real-time factor, задержку первого сегмента и температуру. На текущем ПК обнаружена RTX 4090 Laptop 16 ГБ; её замеры нельзя считать замерами RTX 3060. Стартовая конфигурация хоста: 32 ГБ RAM и SSD с 100 ГБ свободного места как рабочий ориентир, далее уточнить по нагрузке.
