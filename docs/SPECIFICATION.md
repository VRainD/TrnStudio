...
~6 ГБ (RTX 2060; Super часто 8 ГБ) — проектный бюджет VRAM приёмки, не подтверждённый расход. На целевой карте (**RTX 2060**) замерить VRAM, real-time factor, задержку первого сегмента и температуру через профиль Compose `gpu-probe` (`docs/GPU_PROBE.md`). Запас тесный для GigaAM RNNT; при OOM уменьшать сегмент/batch. Замеры с RTX 3060 Ti 16 ГБ или 4090 Laptop нельзя считать замерами 2060. Стартовая конфигурация хоста: 32 ГБ RAM и SSD с 100 ГБ свободного места как рабочий ориентир, далее уточнить по нагрузке.
