"""Long-form audio chunk planning and overlap stitch.

Patterns adapted from dubr1k/GigaAMGUI (src/core/asr/chunking.py) for TrnStudio /
Горизонт. Not a vendored copy of the full GigaAMGUI application. LICENSE files
in this repository are left unchanged.
"""
...
def plan_audio_chunks(
    ...
    *,
    ...
    """Plan decode windows (~20 s) with quiet cut points and overlap context."""
    if sample_rate <= 0:
    ...
    if overlap_seconds < 0 or overlap_seconds >= max_chunk_seconds:
        raise ValueError("overlap_seconds must be < max_chunk_seconds")
        ...
        for index, (nominal_start, nominal_end) in enumerate(
            ...
            chunks.append(
                AudioChunk(
                    ...
                    start_sec=(
                        boundary_start if index == 0 else float(nominal_start) / sample_rate
                    ),
                    end_sec=(
                        boundary_end
                        if index == len(nominal_boundaries) - 2
...
def stitch_overlapping_text(previous: str, current: str) -> tuple[str, str, int]:
    """Drop duplicated prefix of the next window caused by audio overlap."""
    ...
    for size in range(limit, 0, -1):
        ...
        fuzzy = size >= 3 and SequenceMatcher(None, " ".join(left), " ".join(right)).ratio() >= 0.86
